...
4. Optional frame archiving (`archive.raw` via `FrameArchiver` → local JPEG). Must be placed **after** `infer.engine` so it can write `frame_local_path` (absolute) and `frame_url` (relative object key) back onto `InferResult` before publish. Paths use UTC time buckets: `{YYYYMMDD}/{HH}/{mm}/{stream_id}/{ts_ms}_{seq}.jpg` (`frame_layout::objectKey` in `include/archive/FrameLayout.h`).
...
Downstream apps on other hosts use `frame_url` from Kafka / Redis / gRPC payloads and fetch JPEGs via `http://<host>:8082/frames/{frame_url}` (trusted LAN; no auth on nginx in the default compose stack).
