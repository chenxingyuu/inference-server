...
4. Optional frame archiving (`archive.raw` via `FrameArchiver` → local JPEG). Must be placed **after** `infer.engine` so it can write `frame_local_path` back onto `InferResult` before publish. Paths use UTC time buckets: `{YYYYMMDD}/{HH}/{mm}/{stream_id}/{ts_ms}_{seq}.jpg` (`frame_layout::objectKey` in `include/archive/FrameLayout.h`).
