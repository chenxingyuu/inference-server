...
4. Optional frame archiving (`archive.raw` via `FrameArchiver` → local JPEG). Must be placed **after** `infer.engine` so it can write `frame_local_path` (absolute) and `frame_url` (full URL when `frame_archive.public_base_url` is set, else relative object key) back onto `InferResult` before publish. Paths use UTC time buckets: `{YYYYMMDD}/{HH}/{mm}/{stream_id}/{ts_ms}_{seq}.jpg` (`frame_layout::objectKey` in `include/archive/FrameLayout.h`).
...
7. Background services in `main`: heartbeat emission, **`FrameRetentionGc`** (TTL bucket cleanup when `frame_archive.retention.enabled`), and the management plane (see **Management plane** below).

## Frame archive and retention
...
Retention is configured under `frame_archive.retention` (`max_age_minutes`, `scan_interval_seconds`). GC runs even when `frame_archive.enabled` is false, so historical files under `local_dir` are still reclaimed. Expired trees are removed with `remove_all` at day / hour / minute granularity; metrics expose `frames_archive_deleted_total`.

Downstream apps on other hosts use `frame_url` from Kafka / Redis / gRPC payloads. When `frame_archive.public_base_url` is configured (e.g. `http://frame-nginx:8082/frames`), `frame_url` is already a directly fetchable URL and no prefixing is needed. When it is empty, `frame_url` is the relative object key and consumers fetch via `http://<host>:8082/frames/{frame_url}` (trusted LAN; no auth on nginx in the default compose stack).
