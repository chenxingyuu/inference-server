...
export function ModelsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Models"
        subtitle="Loaded inference models"
        action={
          ...
          <table className="data-table">
            ...
            <tbody>
              ...
              {!isLoading && models.length === 0 && (
                <EmptyState message="No models loaded. Load a model to enable inference." />
              )}
              {models.map((m) => (
                <tr key={m.id}>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Load Model" width="max-w-2xl">
        <div className="space-y-4">
          ...
          <Field label="ONNX Path" hint="Required for cpu backend">
            <Input placeholder="/models/yolov8n.onnx" value={form.onnx_path ?? ''}
              onChange={(e) => set('onnx_path', e.target.value)} />
          </Field>

          <Field label="Engine Path" hint="Required for trt / ascend backend">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {load.isPending ? 'Loading…' : 'Load Model'}
            </button>
          </div>
        </div>
      </Modal>
